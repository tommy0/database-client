...
var
  ...
  s: String;
begin
  myLookup:=DataModule1.TOrders.Lookup('Номер заказа' , Edit1.Text,
            'Номер заказа; Код сотрудника; Код клиента; Ставка налога');
  if VarType(myLookup) = varNull then
    ShowMessage('Заказ с таким номером не найден')
  else
    if VarType(myLookup) = varEmpty then
      ...
      begin
        //s:=myLookup[0] + #13 + myLookup[1] + #13 +
          ...
          s:= 'Номер заказа: '+ IntToStr(MyLookup[0])  + #13 +'Код сотрудника: ' + IntToStr(mylookup[1])
          +#13  +  'Код клиента: ' + IntToStr(myLookup[2]) + #13 + 'Ставка Налога: '  + IntToStr(mylookup[3]);
        ShowMessage(s);
      end;
end;
